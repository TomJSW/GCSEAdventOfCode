...
//                getMySession().signOut();
    ...
    public static final class SignUpForm extends Form<Void> {
        ...
        public final void onSubmit() {
            Pattern pattern = Pattern.compile("^[a-zA-Z0-9_]+$"); // Check not all underscores?
            ...
            if (!pattern.matcher(username).matches()) {
                ...
                String errmsg = getString("loginErrorInvalidUsername", null,
                        "Only letters, number, underscore in user name");
                error(errmsg);
                return;
            }
            ...
            if (session.signUp(username, password)) {
                ...
                if (session.userExists(getUsername())) {
                    errmsg = getString("loginError", null, "Unable to sign you up - that user name already exists");
                } else {
